 ...
 /***********************ajax調用database.php連線取得資料庫設定數值***********************/
   $(function () {
             $.ajax({
                 ...
                 {                
                  console.log(data);
                  console.log("場景捲動速度："+parseInt(data[0]['gameSpeed']));
                  console.log("角色重量："+parseInt(data[0]['playerGravity']));
                  console.log("水管上下間隙："+parseInt(data[0]['gapValue']));
                  console.log("小鳥飛行高度："+parseInt(data[0]['birdFly']));  
...
//                  console.log("等級5："+parseInt(data[0]['level5']));     
     ...
     /******************************能從外部變更的變數*****************************/                      
                  gameSpeedValue = parseInt(data[0]['gameSpeed']);
                  birdGravityValue = parseInt(data[0]['playerGravity']);
                  gapValue = parseInt(data[0]['gapValue']);
                  birdFly = parseInt(data[0]['birdFly']);   
